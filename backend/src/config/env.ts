...
const schema = z.object({
  ...
  , TURN_URL: z.string().max(1000).optional(), TURN_SHARED_SECRET: z.string().min(16).optional(), TURN_TTL_SECONDS: z.coerce.number().int().min(300).max(86400).default(3600)
});
...
if (env.NODE_ENV === "production") {
  ...
  if (Boolean(env.TURN_URL) !== Boolean(env.TURN_SHARED_SECRET)) throw new Error("TURN_URL and TURN_SHARED_SECRET must be configured together");
}
