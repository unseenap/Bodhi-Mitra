...
function LeaderMessage({ leader }: { leader: Leader }) {
  return <article className="leadership-message"><img className="leadership-message__portrait" src={leader.image} alt={leader.imageAlt} /><header><p>{leader.title}</p><h3>{leader.name}</h3><span>{leader.affiliation}</span></header><div className="leadership-message__body">{leader.message.map(paragraph => <p key={paragraph}>{paragraph}</p>)}</div></article>;
}

export function AboutPageRedesign() {
  return <div className="about-rebuild">
    <section className="about-rebuild__hero"><div className="about-rebuild__hero-copy"><span>About Bodhi-Mitra</span><h1>Built by GBU, for GBU students.</h1><p>A safer path to timely, private, and compassionate psychological support.</p><Link to="/experts">Meet our experts <ArrowRight /></Link></div><div className="about-rebuild__hero-image"><img src="/images/GBUBG2.png" alt="Gautam Buddha University campus" /></div></section>
    <section className="leadership-layout leadership-layout--vc"><LeaderMessage leader={viceChancellor} /><div className="leadership-layout__context"><div className="leadership-about"><img src="/images/pschylogo.svg" alt="Bodhi-Mitra logo" /><h2>About Bodhi-Mitra</h2><p>A student-first mental health initiative by Gautam Buddha University, designed for safe and private support.</p></div><div className="leadership-story"><Brain weight="duotone" /><h3>Our Story</h3><p>Bodhi-Mitra was created to close the gap between students who need support and the qualified professionals ready to help.</p></div><div className="leadership-mini"><Compass weight="duotone" /><h3>Our Name</h3><p><strong>Bodhi</strong> means awakening. <strong>Mitra</strong> means friend.</p></div><div className="leadership-mini"><Medal weight="duotone" /><h3>GBU Initiative</h3><p>Developed through the Department of Psychology and Mental Health.</p></div></div></section>
    <section className="leadership-layout leadership-layout--pair"><LeaderMessage leader={dean} /><LeaderMessage leader={hod} /></section>
    <section className="about-rebuild__standards"><header><h2>Safety is part of the architecture</h2><p>Privacy and accountability are designed into every role and support pathway.</p></header><div>{standards.map(({ icon: Icon, title, body }, index) => <article key={title} className={`about-standard about-standard--${index + 1}`}><Icon weight="duotone" /><h3>{title}</h3><p>{body}</p></article>)}</div></section>
    <section className="about-rebuild__governance"><img src="/images/logo.svg" alt="Gautam Buddha University" /><div><HandHeart weight="duotone" /><h2>University-governed support</h2><p>Psychologists join the panel only after an administrator confirms their qualifications and professional details.</p></div><Link to="/contact">Contact the team <ArrowRight /></Link></section>
  </div>;
}
