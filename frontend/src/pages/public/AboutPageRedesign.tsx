import { ArrowRight, Brain, Compass, HandHeart, Heart, LockKey, Medal, Quotes, ShieldCheck, UsersThree } from "@phosphor-icons/react";
import { Link } from "react-router-dom";
import { AnimatedGradientText, BlurFade, NumberTicker, ShineBorder } from "../../components/magicui/ProfileMagic";
...
const hod: Leader = {
  ...
  title: "HOD, Psychology Department",
  affiliation: "Gautam Buddha University",
  image: "/images/leadership/hod.svg",
  imageAlt: "Dr. Anand Pratap Singh",
  message: [
...
function LeaderMessage({ leader }: { leader: Leader }) {
  return (
    <BlurFade className="leadership-message-reveal">
      <article className="leadership-message">
        <ShineBorder duration={18} />
        <div className="leadership-message__identity">
          <img className="leadership-message__portrait" src={leader.image} alt={leader.imageAlt} />
          <header><Quotes weight="duotone" /><p>{leader.title}</p><TextReveal as="h2" text={leader.name} delay={.04} /><span>{leader.affiliation}</span></header>
        </div>
        <div className="leadership-message__body">{leader.message.map(paragraph => <p key={paragraph}>{paragraph}</p>)}</div>
      </article>
    </BlurFade>
  );
}
...
export function AboutPageRedesign() {
  return <div className="about-rebuild">
    <section className="about-rebuild__hero"><BlurFade className="about-rebuild__hero-copy"><ShinyText text="About Bodhi-Mitra" speed={7} /><TextReveal as="h1" text="Built by GBU, for GBU students." delay={.05} /><p>A safer path to timely, private, and compassionate psychological support.</p><Link to="/experts">Meet our experts <ArrowRight /></Link></BlurFade><BlurFade className="about-rebuild__hero-image" delay={.12}><SpotlightCard className="about-hero-spotlight"><img src="/images/GBUBG2.png" alt="Gautam Buddha University campus" /></SpotlightCard></BlurFade></section>
    <section className="about-rebuild__impact" aria-label="Bodhi-Mitra service principles"><BlurFade><span><strong><NumberTicker value={24} /><small>/7</small></strong><p>Support vision</p></span></BlurFade><BlurFade delay={.04}><span><strong><NumberTicker value={3} /></strong><p>Ways to connect</p></span></BlurFade><BlurFade delay={.08}><span><strong><NumberTicker value={4} /></strong><p>Safety standards</p></span></BlurFade><BlurFade delay={.12}><span><strong><NumberTicker value={1} /></strong><p>University community</p></span></BlurFade></section>
    <BlurFade className="about-leadership-intro"><Quotes weight="duotone" /><div><ShinyText text="Messages of commitment" speed={8} /><TextReveal as="h2" text="Leadership behind the mission" delay={.04} /><p>Three perspectives, one commitment to student wellbeing and compassionate support.</p></div></BlurFade>
    <section className="leadership-layout leadership-layout--vc"><LeaderMessage leader={viceChancellor} /><BlurFade className="leadership-layout__context" delay={.08}><SpotlightCard className="leadership-about"><ShineBorder duration={16} /><img src="/images/pschylogo.svg" alt="Bodhi-Mitra logo" /><h2><AnimatedGradientText speed={1.3} colorFrom="#ffffff" colorTo="#c4b5fd">About Bodhi-Mitra</AnimatedGradientText></h2><p>A student-first mental health initiative by Gautam Buddha University, designed for safe and private support.</p></SpotlightCard><div className="leadership-story"><Brain weight="duotone" /><h3>Our Story</h3><p>Bodhi-Mitra was created to close the gap between students who need support and the qualified professionals ready to help.</p></div><div className="leadership-mini"><Compass weight="duotone" /><h3>Our Name</h3><p><strong>Bodhi</strong> means awakening. <strong>Mitra</strong> means friend.</p></div><div className="leadership-mini"><Medal weight="duotone" /><h3>GBU Initiative</h3><p>Developed through the Department of Psychology and Mental Health.</p></div></BlurFade></section>
