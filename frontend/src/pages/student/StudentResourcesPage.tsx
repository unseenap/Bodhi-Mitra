...
const groundingSteps = [
  { number: 5, sense: "See", text: "Name five things you can see.", Icon: Eye },
  {
    number: 4,
    sense: "Touch",
    text: "Notice four things you can feel.",
    Icon: Hand,
  },
  {
    number: 3,
    sense: "Hear",
    text: "Listen for three different sounds.",
    Icon: Headphones,
  },
  {
    number: 2,
    sense: "Smell",
    text: "Find two scents around you.",
    Icon: Wind,
  },
  {
    number: 1,
    sense: "Taste",
    text: "Notice one taste in this moment.",
    Icon: Sparkle,
  },
] as const;
...
const helplines = [
  {
    name: "GBU Counselling Centre",
    number: "+91 9650257255",
    href: "+919650257255",
    tag: "Campus",
  },
  {
...
export function StudentResources() {
  ...
  const progress = breathing
    ? ((phaseDuration - remaining + 1) / phaseDuration) * 360
    : 0;
    ...
    <section className="student-page wellbeing-page">
      <BlurFade className="wellbeing-hero">
        ...
        <div className="wellbeing-hero__copy">
          <span>
            <Sparkle weight="fill" /> Your private wellbeing toolkit
          </span>
          <TextReveal
            as="h1"
            text="A quieter moment starts here."
            delay={0.05}
          />
          <p>
            Use a short guided exercise, return to the present, or reach direct
            support. Choose only what feels helpful right now.
          </p>
          <nav>
            <a href="#breathing">
              Start breathing <ArrowRight />
            </a>
      ...
      <BlurFade className="wellbeing-note" delay={0.08}>
        ...
        <p>
          <strong>Use these tools at your own pace.</strong> Your activity on
          this page is not saved to your profile.
        </p>
      </BlurFade>
      ...
      <div className="wellbeing-grid">
          ...
          <SpotlightCard className="wellbeing-card breathing-studio">
            ...
            <header id="breathing">
              ...
              <div>
                ...
                <h2>4-4-6-2 breathing</h2>
              </div>
              <b>
                <Timer />{" "}
                {cycles ? (
                  <>
                    <NumberTicker value={cycles} />{" "}
            ...
            <div
              className={`breathing-stage breathing-stage--${breathing ? "active" : "idle"}`}
            >
              <div
                className="breathing-orbit"
                style={
                  { "--breathing-progress": `${progress}deg` } as CSSProperties
                }
              >
                <span>
                  <Wind weight="duotone" />
              ...
              <div className="breathing-instruction" aria-live="polite">
                <small>
                  {breathing ? `Step ${phase + 1} of 4` : "Ready when you are"}
                </small>
                <strong>
                  {breathing
                    ? breathingPhases[phase].label
                    : "Find a comfortable position"}
                </strong>
                <b>{breathing ? remaining : "4 · 4 · 6 · 2"}</b>
                <p>
                  {breathing
                    ? "Let the circle guide your pace. There is no need to force the breath."
                    : "A slower exhale can help your body settle after a stressful moment."}
                </p>
              </div>
            </div>
            <footer>
              {breathing ? (
                <Button variant="secondary" onClick={() => setBreathing(false)}>
                  <Pause weight="fill" /> Pause exercise
                </Button>
              ) : (
                <Button onClick={startBreathing}>
                  <Play weight="fill" /> Begin guided breathing
                </Button>
              )}
        ...
        <BlurFade className="wellbeing-grid__grounding" delay={0.16}>
          <SpotlightCard className="wellbeing-card grounding-studio">
            ...
            <p>
              Move through each sense. Tap a step when you have noticed
              something.
            </p>
              ...
              {groundingSteps.map(({ number, sense, text, Icon }) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => toggleGrounding(number)}
                    aria-pressed={done}
                    key={number}
                  >
                    <b>{done ? <Check weight="bold" /> : number}</b>
                    <Icon />
                    <span>
                      <strong>{sense}</strong>
                      <small>{text}</small>
            ...
            <footer>
              <span>
                <NumberTicker value={groundingDone.length} /> of 5 complete
              </span>
              {groundingDone.length > 0 && (
                <button onClick={() => setGroundingDone([])}>Reset</button>
              )}
            </footer>
          </SpotlightCard>
        </BlurFade>
        ...
        <BlurFade className="wellbeing-grid__affirmation" delay={0.2}>
          <SpotlightCard className="wellbeing-card affirmation-studio">
            ...
            <button
              onClick={() =>
                setAffirmation((value) => (value + 1) % affirmations.length)
              }
            >
              Show another <ArrowRight />
            </button>
          </SpotlightCard>
        </BlurFade>
        ...
        <BlurFade className="wellbeing-grid__help" delay={0.24}>
          <SpotlightCard className="wellbeing-card support-prompt">
            ...
            <div>
              <small>Need a person instead?</small>
              <h2>You do not have to manage this alone.</h2>
              <p>
                A university-approved psychologist can join you in a private
                support session.
              </p>
            </div>
            <Link to="/student">
      ...
      <BlurFade className="helpline-directory" delay={0.28}>
        <header id="helplines">
          <div>
            <span>
              <Phone weight="fill" /> Direct support
            </span>
            <h2>Call when you need a human voice</h2>
            <p>
              If you may be in immediate danger, call 112 or ask someone nearby
              to stay with you.
            </p>
          </div>
          <b>Tap any number to call</b>
        </header>
        <div>
          {helplines.map((line, index) => (
            <a href={`tel:${line.href}`} key={line.name}>
              <span>{String(index + 1).padStart(2, "0")}</span>
              <div>
                <small>{line.tag}</small>
      ...
      <p className="wellbeing-disclaimer">
        These resources support wellbeing and do not provide a diagnosis or
